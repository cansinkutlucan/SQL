...
# ÖRNEĞİN : writer, novel, consumer : entityler,  creates ve buys: relationship
...
# 1.Chen's notation 2.Crow's foot notation(Biz ikinciyi(Crow's foot notationı) kullanacağız)
######
...
# Normal Forms
    ...
    # Second normal form
        # 1.Sağlamamız gereken şey 2.fazda non-key bir sütunun ancak 2 PK bir araya gelerek o non-key i identify etmesi gerekiyor
...
# ALTER       : Sütunların yapısında değişiklik yapar(Değerlerini değiştirmiyoruz. Onu UPDATE ile yapıyoruz)
    # ADD          : ALTER TABLE table_name ADD column_name datatype   # Sütun ekleme
    # ALTER COLUMN : ALTER TABLE table_name ALTER COLUMN column_name   # Sütun değiştirme
    # DROP COLUMNS : ALTER TABLE table_name DROP COLUMN column_name    # Sütun silme , Sütun drop etme
# DROP        : Tabloyu drop ediyor
    # DROP TABLE table_name # Tablo silme, Tablo drop etme
...
# SELECT INTO : Bir query nin döndürdüğü sonuçları başka tabloya koyar, tablo kopyalama
...
# UPDATE : Bir tablodaki değerleri güncellememizi sağlar
    ...
    # Örn: UPDATE table_name SET Name = "Ali" WHERE ID=1] # tablodaki bir değeri güncelleme
# DELETE
    # DELETE ALL ROWS       : DELETE FROM table_name:   # Bunu pek kullanmamalıyız
    # DELETE WITH CONDITION : DELETE FROM table_name WHERE conditions # Tablodan bir değer silme
    # DELETE TOP ... FROM   : DELETE TOP 20 FROM table_name WHERE conditions

# IMPORTANT NOTE: To remove specific rows, use DELETE . To remove all rows from a large table and leave the table structure, use 
# .. TRUNCATE TABLE . It's faster than DELETE . To remove an entire table, including its structure and data, use DROP TABLE
"""
TRUNCATE
TRUNCATE Command is a Data Definition Language operation. It is used to remove all the records from a table. 
It deletes all the records from an existing table but not the table itself.

DELETE
It is used to delete existing records from an existing table. We can delete a single record or multiple records depending on 
the condition specified in the query.

https://afteracademy.com/blog/what-is-the-difference-between-truncate-delete-and-drop-statements
"""
...
#%% SQL-3
########## INSERT / INSERT INTO
# Tabloya değer eklemek için kullanılır
# INSERT INTO table_name (columns) VALUES (value11, value12,...), (value21, value22, ...), ...
# NOT: INSERT INTO yerine INSERT yazabiliriz
...
-----------------------------------------
INSERT INTO Person.Person  VALUES (889623212466,'Kerem',"Yılmaz")  -- Sütun ismi yazmazsam tüm sütunlara insert edebilir. Ancak;(Alta bakalım)
...
INSERT INTO Person.Person (Person_LastName,SSN, Person_FirstName) VALUES ("Yalnız",689623512466,'Ahmet') -- Sütun sırasını değiştirebilirim ama 
    -- .. values sırasını da değiştirmeliyim
----------------------------------------
...
INSERT INTO Person.Person_Mail (Mail, SSN) VALUES ('zehtek@gmail.com',75056659595),('meyet@gmail.com',13056659595),('metsak@gmail.com',24056659595)
----------------------------------------
...
########## SELECT ... INTO
# Yeni tablo oluşturma ve eski tablodaki bilgileri yeni tabloya aktarma.(Bir tabloyu kopyalama)
...
# Diagramda bir tabloya sağ tık -- > table view -- modify custom --> 
    # .. --> (sol taraftaki "Data Type" ı sağ tarafa alıp tabloyu diagramda dtype ları ile birlikte görürüz ama sağ tık tekrar
    # .. table view den standart ı seçmeliyiz vs vs) 
...
# Date Tipleri: time ,date, smalldatetime, datetime, datetime2, datetimeoffset,

##########  GETDATE() : Sistemimizin/Bilgisayarın tarihi ve zamanını datetime tipinde getirir.
...
-- 6 tane sütunu var. Her sütunun veritipi farklı
-- Şimdi inceleyelim
...
12:00:00.0000000	2021-07-17	2021-07-17 00:00:00	2021-07-17 00:00:00.000	2021-07-17 00:00:00.0000000	2021-07-17 00:00:00.0000000 +00:00
"""
...
-- 6            : "Date only format tablosundaki" Dönüştürmek istediğim stil no(str 462 daki linke bkz) 
"""
...
FROM t_date_time --- (str 430 bkz. Bu tabloyu oluşturduk ve str 447-452 insert yapıldı)
...
"""
########## DATEDIFF: IKI TARIH/ZAMAN ARASINDAKI FARKI ALMAK(gün farkı, dakika farkı, saat farkı vs)
"""
...
# NOT: SELECT DATEADD(month, DATEDIFF(month, 0, GETDATE()), 0)  -- Instead of GetDate you can put any date. # Ayın ilk gününü getirme
# NOT: SELECT DATEFROMPARTS(YEAR(@mydate),MONTH(@mydate),1) # Ayın ilk gününü getirme
"""
...
# PATINDEX('%pattern%', input_string)           : Pattern indexi Bir pattern aradığımız zaman örn:
                                                # .. Bir sütunda "h" ve "m" harflerinin yan yana olduğu pattern i aramak istersek
...
SELECT PATINDEX('%R', 'CHARACTER')        -- Output : 9   -- String R ile biterse "R" nin indexi(İçindeki R ler önemli değil. Stringin bittiği R önemli)
...
# NOT: Sadece nümerikleri çekmek istersek;
...
# STRING_SPLIT : A table-value function: İçine yazılan her bir string i tek bir sütunda tablo kaydı olarak döndürür
# NOT: Öncekiler(Yukarıdakiler) tek bir değer döndürüyordu, A table-value function bir tablo döndürecek
# NOT: Altta bazı kodlar SQL de hatalı gibi görünüyor ama SQL Server da ama çalışıyor
"""
...
# TRIM()   : Verinin başında ve sonundaki boşlukları ya da verilen ifadeleri temizler
...
SELECT TRIM(        '       CHAR ACTER X')   -- output: CHAR ACTER X Tek tırnaktan önce yazılan boşlukların anlamı yok, tırmaktan sonra text ifadenin başladığını biliyor fonksiyon
----------------------------------------
-- TRIM diğer kullanım
...
# NOT: Verilen ifadeleri atmıyor MSSQL de ama internette şöyle örnekler var; --> # SELECT LTRIM (‘000325400’, ‘0’); Output: 325400
...
# STR()     : Nümerik sayının string e çevrilmesini istersek kullanıyoruz. NOT: String fonksiyonu bize 10 karakterlik text döndürüyor.
"""
...
# CONVERT() : Tanımlanmış cast işlemleri. Tarih veriyorum. Bunların içinden  belli parçasını getir. Veri tipini döndürmeyede yarar. Örneklerde göreceğiz
...
SELECT CONVERT(NVARCHAR, GETDATE(),112)      -- Output: 20220606                 -- NVARCHAR veritipinde bir sonuç getirdi   -- 112 formatında(YYYYMMDD)(bkz str 466)   
-----------------------------------------
...
# ISNULL()    : 2 parametre için null olmayan ilk değeri getiriyor bir nevi
              # 2 parametre yazacağız mesela birinci parametre NULL sa ikinci parametreyi getirecek
              # ÖRN: Öğrenci yaş ortalamasını al diyeceğiz mesela. NULL olmayan değerlerin ortalamasını al derken bunu kullanacağız.
# NULLIF()    : 2 değerin birbirine eşit olup olmaması. Eşitse NULL eşit değilse ilk dolu olanı getirmesini istersek bu fonksiyonu kullanıyoruz
...
-- Buradaki 0 yazan yer : 0 ya da 1 değer alıyor    -- default olarak 0 alıyor yukarı yuvarlar. 1 yazarsak aşağı yuvarlar
-----------------------------------------
...
# 2.LEFT JOIN  : 2 tane tabloda soldaki tabloyu baz alarak değerleri getiriyor. 
    # # 1. tablodaki(soldaki) tüm değerler gelecek. 2. tablodaki değerler 1. tablodada varsa getirir, yoksa NULL getirir
...
# 3.RIGHT JOIN   : 2 tane tabloda sağdaki tabloyu baz alarak değerleri getiriyor.
    # 2. tablodaki(sağdaki) tüm değerler gelecek. 1. tablodaki değerler 2. tablodada varsa getirir, yoksa NULL getirir
...
# DIKKAT: VIEW içerisinden ORDER BY kullanamayız(kullansaydık VIEW OLUŞMAYACAKTI). VIEW oluştuktan sonra ORDER BY ı kullanabiliriz
...
##########
# 1.Having Clause
# Group by lı sorgu sonucunda olan filtrelemelerde aggregation kullanıyorsak filtreyi WHERE yerine HAVING ile yapıyoruz
...
-- GROUPING SETS ile hemen hemen(biri 50 rows, diğeri 54 rows-NULL toplam eklenince) aynı çıktı altta
...
# Pivot yapacağımız tabloyu subquery içine alıp select * from (subquery) A PIVOT ... şeklinde devam ediyoruz
"""
...
------
-- 2. yol sadece "NULL" un yeri değişiyor
SELECT * FROM (SELECT category_id, Model_Year, product_id FROM product.product) A
PIVOT
(count(product_id) FOR Model_Year IN ([2018], [2019], [2020], [2021])
) AS PIVOT_TABLE
UNION ALL
SELECT * FROM (SELECT NULL null_column, product_id, Model_Year FROM product.product) A
PIVOT
(count(product_id) FOR Model_Year IN ([2018], [2019], [2020], [2021])
) AS PIVOT_TABLE
"""
...
-- NOT: Değişkenlerin sırası ve sayısı önemli,UNION da sütun ismini değiştirdiğimizde  üstündeki sütun ismini baz alır
-----------------------------------------
...
-- Öncekinde UNION la yaptığımızda 105 rows gelmişti(bkz str 1026)
...
order by 1
----------------------------------------
-- Üstte pivot table ile gösterilen çıktıyı başka bir şekilde gösterelim
...
order by 1
----------------------------------------
-- SORU:model_year ı hem 2018 yılında hem de 2019 yılında olan brand_id ler sorusu vardı INTERSECT te
...
# Alttaki örnekte order_status = 1 yerine order_id = 1 yazabiliriz
"""
...
-----------------------------------------
-- Soru: Aynı siparişte hem mp4 player, hem Computer Accessories hem de Speakers kategorilerinde ürün siparişi veren müşterileri bulunuz.
...
# 1. sorguda subqueries de order_item dan sum(list_price* quantity*(1-discount)) hesaplanmış
...
-- 2021 yılında yapılmış olan ürünleri ve  Game, gps or Home Theater dışında olanlar   
...
select * from sale.customer A where exists (select * from sale.orders B where B.order_date > '2020-01-01' and A.customer_id = B.customer_id) 
-- 2020 ocak 1 den sonra sipariş veren müşteri

select * from sale.customer A where  not exists (select * from sale.orders B where B.order_date > '2020-01-01' and A.customer_id = B.customer_id) 
-- 2020 ocak 1 den sonra sipariş vermiş OLMAYAN müşteri
...
.. WF de belirlediğiniz grubun ortalamasına göre farklı sonuçlar döndürüyor. WF de order by gerekiyor genelde.
Performance : Group by biraz yavaş, WF daha hızlıdır(Genelde)
Dependency on selected Field: Group by yaparken bilgilerin seçilen alana bağlıdır. Bazı bilgiler kaybolur(Ortalama alırken ortalaması alınan
.. bilgiler) çıktıda WF de bu bağımsızdır
"""
# -----------------------------------------
"""
...
# partition current row dan itibaren sona kadar olan satır bu satır benim frame im olsun diyebilirim(current row dahil)
...
-- 4 WF:ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) prev_with_current, : Bu default değerdir WF de. Default olduğu için bir 
...
select store_id, most_stocked_prod, FIRST_VALUE(most_stocked_prod) OVER(ORDER BY most_stocked_prod DESC) MSP_W  from (Select distinct store_id, 
FIRST_VALUE(product_id) OVER(PARTITION BY store_id ORDER BY quantity DESC) most_stocked_prod
FROM product.stock) A
-----------------------------------------
...
-- NOT: range ile rows hemen hemen aynı işlemleri yapıyor
    -- rows : unbounded preciding/following gibi keyword lerle kullanıp satır sayısı belirtmek istiyorsanız kullanıyoruz
...
-- default olarak  ORDER BY datepart(ISO_WEEK, b.order_date) da  --> rows between unbounded preceding and current row geldi. O yüzden cumulative oldu.
-- .. Countta da yukar  1,2,3,4..  diye satırları rows between unbounded preceding and current row mantığıyla saymıştık (CTRL+F --> 4 WF)
-------------------------------------------
...
#%% SQL-13
# DATABASE INDEX
# Bir tabloda belli alanlara yapılan sorgular daha fazla ise Mesela: Kişi tablosunda sürekli isim üzerinden sorgu yapılıyor). 
# .. Bu alanlara index atıyorsunuz. Bize fayda sağlıyor Indexler database seviyesinde oluyor. Yani DB deki bütün sütunlara 
# .. index atalım diyemiyoruz
...
-- Select Coat: .. 
-- Table scan: Kullanmış olduğu yöntem. Bu ekranda en üstte. (Cluster yapınca burası "Clustered Index seek" olacak. Şuan scan olarak görünüyor)
...
select ad from website_visitor where ad = 'visitor_name17';
-- "execution plan"a bakalım. Index seek. Artık en alttaki "leaf" leri okumak zorunda değil(B-tree de)
...
# Stored procedures : DB tarafında insert,update, alter vs gibi işlemlerle ilgili kullanıyoruz
    # .. Örneğin: denormalize verisetini normalize hale getirmeniz gerekiyor. Bunu sürekli yapmanız gerekiyor. Siz bir stored procedures
    # .. oluşturursunuz. Bunu kaydediyorsunuz. Bu günde 1 çalışsın ya da şu tablo üzerinde çalışsın diyebiliyoruz
...
# Procedure de bir şey silmek ya da değiştirmek istersek drop edip sonra tekrar oluşturabiliriz
"""
...
"""
# 2 tablomuz var. Order tablosu sürekli değişecek. Öyle bir procedure yazayım ki order_tbl deki satır sayısını döndürsün
"""
...
# Herhangi bir tarih girip. O tarihteki sipariş sayısını döndürsün
...
-- Input alarak almış olduğu metni büyük harfe çeviren fonksiyon
CREATE FUNCTION fnc_uppertext(@inputtext varchar (MAX))
RETURNS VARCHAR (MAX)  -- RETURNS INT --> Eğer integer yazarsam bu sefer altta fonksiyonun içine de integer bir ifade koymalıyım çalışması için
AS
BEGIN
...
-- NOT: Burada As den sonra begin ve end arasında return edecek şeyi yazıyorum. 
-- .. Direk begin ve end kullanırsam hata. O yüzden önce return edeceğim şeye geçiyorum
-------------------------------------------------
...
SET @input = 11
SELECT @modulus = @input % 2
PRINT @modulus  -- ya da SELECT @modulus
------------------
...
------------------------------------------
create FUNCTION dbo.fnc_tekcift(@input int)
...
-- begin ve end i belirtmemiz gerekiyor

"""
"""
-- Fonksiyon haline getirmek için; https://www.sqlservercentral.com/forums/topic/create-function-with-a-while-loop
...
RETURN;
END
select * from dbo.fn_list_the_number(50)
"""
# ----------------------------------------------------
"""
...
WHERE A.ORDER_ID = B.ORDER_ID
------------------------------------
create FUNCTION dbo.fnc_orderstatus(@input int)
RETURNS nvarchar(max)
AS
BEGIN
	declare
	...
	select	@result =
				case
					when B.DELIVERY_DATE < A.EST_DELIVERY_DATE then 'EARLY'
					when B.DELIVERY_DATE > A.EST_DELIVERY_DATE then 'LATE'
					when B.DELIVERY_DATE = A.EST_DELIVERY_DATE then 'ON TIME'
				else NULL end
